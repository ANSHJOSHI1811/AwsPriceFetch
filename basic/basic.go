		...
		for termType, termDetails := range termData {
			...
			for priceKey, priceDetails := range termDetails.PriceDimensions {
				...
				termEntry := models.Price{
					...
					RateCode:      priceDetails.RateCode,
				...
				if err := db.Create(&termEntry).Error; err != nil {
					log.Printf("Failed to insert term for SKU %s and RateCode %s: %v", skuCode, priceDetails.RateCode, err)
					continue
				}

				log.Printf("Successfully inserted term for SKU %s and RateCode %s", skuCode, priceDetails.RateCode)
